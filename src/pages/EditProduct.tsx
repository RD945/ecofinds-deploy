                          ...
                          <div className="mt-4 grid grid-cols-3 sm:grid-cols-5 gap-2">
                              {existingImages.map((image, index) => (
                                   <div key={`existing-${image.id}`} className="relative aspect-square">
                                      <img src={image.url || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/images/${image.id}`} alt={`Existing Preview ${index}`} className="w-full h-full object-cover" />
                                      <button
                                        type="button"
